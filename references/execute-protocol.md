...
4b. **Worktree isolation (REQ-WORKTREE):** If `worktree_isolation` is not `"off"` in config:
   ...
   - Create worktree: `WPATH=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-create.sh {phase} {plan} 2>/dev/null || echo "")`. If `WPATH` is empty, log warning and continue without worktree for this plan.
   - If `WPATH` is non-empty:
     ...
     - Fetch targeting JSON: `WTARGET=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-target.sh "$WPATH" 2>/dev/null || echo "{}")`.
     - Register agent mapping: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-agent-map.sh set "dev-{plan}" "$WPATH" {phase} {plan} 2>/dev/null || true`.
...
6. **Crash recovery:** If `.vbw-planning/.execution-state.json` exists with `"status": "running"`, update plan statuses to match current SUMMARY.md state.
   - **Event Recovery (REQ-17):** If `event_recovery=true` in config, attempt event-sourced recovery first:
     `RECOVERED=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/recover-state.sh {phase} 2>/dev/null || echo "{}")`
...
8. **Event Log (REQ-16, graduated, always-on):**
   - Log phase start: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh phase_start {phase} 2>/dev/null || true`
...
9. **Snapshot Resume (REQ-18):** If `snapshot_resume=true` in config:
   - On crash recovery (execution-state.json exists with `"status": "running"`): attempt restore:
     `SNAPSHOT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/snapshot-resume.sh restore {phase} {preferred-role} 2>/dev/null || echo "")`
...
10. **Schema Validation (REQ-17, graduated, always-on):**
   - Validate each PLAN.md frontmatter before execution:
     `VALID=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/validate-schema.sh plan {plan_path} 2>/dev/null || echo "valid")`
   - If `invalid`: log warning `⚠ Plan {NN-MM} schema: ${VALID}` — continue execution (advisory only).
   - Log to metrics: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh schema_check {phase} {plan} result=$VALID 2>/dev/null || true`
...
- Before creating agent teams, assess each plan:
  ```bash
  RISK=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/assess-plan-risk.sh {plan_path} 2>/dev/null || echo "medium")
...
- If `RISK=low` AND `TASK_COUNT<=3` AND effort is not `thorough`: force turbo execution for this plan (no team, direct implementation). Log routing decision:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh smart_route {phase} {plan} risk=$RISK tasks=$TASK_COUNT routed=turbo 2>/dev/null || true`
- Otherwise: proceed with normal team delegation. Log:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh smart_route {phase} {plan} risk=$RISK tasks=$TASK_COUNT routed=team 2>/dev/null || true`
...
- Before context compilation, detect relevant package paths:
  `PACKAGES=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/route-monorepo.sh {phase_dir} 2>/dev/null || echo "[]")`
- If non-empty array (not `[]`): pass package paths to context compilation for scoped file inclusion.
  Log: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh monorepo_route {phase} packages=$PACKAGES 2>/dev/null || true`
- If empty or error: proceed with default (full repo) context compilation.

**Control Plane Coordination (REQ-05):** If ``!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh` exists:
- **Once per plan (before first task):** Run the `full` action to generate contract and compile context:
  ```bash
  CP_RESULT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh full {phase} {plan} 1 \
...
- **Before each task:** Run the `pre-task` action:
  ```bash
  CP_RESULT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh pre-task {phase} {plan} {task} \
...
- **After each task:** Run the `post-task` action:
  ```bash
  CP_RESULT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh post-task {phase} {plan} {task} \
...
`bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/compile-context.sh {phase} dev {phases_dir} {plan_path}`
...
- After context compilation, enforce per-role token budgets. Pass the contract path and task number for per-task budget computation:
  ```bash
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/token-budget.sh dev {phase-dir}/.context-dev.md {contract_path} {task_number} > {phase-dir}/.context-dev.md.tmp && mv {phase-dir}/.context-dev.md.tmp {phase-dir}/.context-dev.md
  ```
...
- Same for QA context: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/token-budget.sh qa {phase-dir}/.context-qa.md {contract_path} {task_number} > ...`
...
DEV_MAX_TURNS=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh dev .vbw-planning/config.json "{effort}")
...
QA_MAX_TURNS=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh qa .vbw-planning/config.json "{effort}")
...
- **Per plan:** Assess risk and resolve gate policy:
  ```bash
  RISK=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/assess-plan-risk.sh {plan_path} 2>/dev/null || echo "medium")
  GATE_POLICY=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-gate-policy.sh {effort} $RISK {autonomy} 2>/dev/null || echo '{}')
  ```
...
- Log to metrics: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh gate_policy {phase} {plan} risk=$RISK qa_tier=$QA_TIER approval=$APPROVAL 2>/dev/null || true`
...
Schema ref: ``!`echo $CLAUDE_PLUGIN_ROOT`/references/handoff-schemas.md`
...
- **On message receive** (from any teammate): validate before processing:
  `VALID=$(echo "$MESSAGE_JSON" | bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/validate-message.sh 2>/dev/null || echo '{"valid":true}')`
  If `valid=false`: log rejection, send error back to sender with `errors` array. Do not process the message.
- **On message send** (before sending): agents should construct messages using full V2 envelope (id, type, phase, task, author_role, timestamp, schema_version, payload, confidence). Reference ``!`echo $CLAUDE_PLUGIN_ROOT`/references/handoff-schemas.md` for schema details.
...
- At plan start: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh plan_start {phase} {plan} 2>/dev/null || true`
- At agent spawn: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh agent_spawn {phase} {plan} role=dev model=$DEV_MODEL 2>/dev/null || true`
- At agent shutdown: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh agent_shutdown {phase} {plan} role=dev 2>/dev/null || true`
- At plan complete: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh plan_end {phase} {plan} status=complete 2>/dev/null || true`
- At plan failure: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh plan_end {phase} {plan} status=failed 2>/dev/null || true`
- On error: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh error {phase} {plan} message={error_summary} 2>/dev/null || true`
...
- `artifact_written`: after writing/modifying a file: `log-event.sh artifact_written {phase} {plan} path={file} task_id={id}`
  - Also register in artifact registry: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/artifact-registry.sh register {file} {event_id} {phase} {plan}`
...
- After each plan completes (SUMMARY.md verified):
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/snapshot-resume.sh save {phase} .vbw-planning/.execution-state.json {agent-role} {trigger} 2>/dev/null || true`
...
- At phase start: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh execute_phase_start {phase} plan_count={N} effort={effort}`
- At each plan completion: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh execute_plan_complete {phase} {plan} task_count={N} commit_count={N}`
- At phase end: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/collect-metrics.sh execute_phase_complete {phase} plans_completed={N} total_tasks={N} total_commits={N} deviations={N}`
...
- **Once per plan (before first task):** Generate contract sidecar:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/generate-contract.sh {plan_path} 2>/dev/null || true`
...
- **Before each task:** Validate task start:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/validate-contract.sh start {contract_path} {task_number} 2>/dev/null || true`
- **After each task:** Validate modified files against contract:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/validate-contract.sh end {contract_path} {task_number} {modified_files...} 2>/dev/null || true`
...
- **Pre-task gate sequence (before each task starts):**
  1. `contract_compliance` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh contract_compliance {phase} {plan} {task} {contract_path}`
  2. **Lease acquisition** (V2 control plane): acquire exclusive file lease before protected_file check:
     `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh acquire {task_id} --ttl=300 {claimed_files...}`
     - Lease conflict → auto-repair attempt (wait + re-acquire), then escalate blocker if unresolved.
  3. `protected_file` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh protected_file {phase} {plan} {task} {contract_path}`
  - If any gate fails (exit 2): attempt auto-repair:
    `REPAIR=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/auto-repair.sh {gate_type} {phase} {plan} {task} {contract_path})`
...
- **Post-task gate sequence (after each task commit):**
  1. `required_checks` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh required_checks {phase} {plan} {task} {contract_path}`
  2. `commit_hygiene` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh commit_hygiene {phase} {plan} {task} {contract_path}`
  3. **Lease release**: release file lease after task completes:
     `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh release {task_id}`
...
- **Post-plan gate (after all tasks complete, before marking plan done):**
  1. `artifact_persistence` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh artifact_persistence {phase} {plan} {task} {contract_path}`
  2. `verification_threshold` gate: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/hard-gate.sh verification_threshold {phase} {plan} {task} {contract_path}`
...
- Use `lease-lock.sh` for all lock operations:
  - Acquire: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh acquire {task_id} --ttl=300 {claimed_files...} 2>/dev/null || true`
  - Release: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh release {task_id} 2>/dev/null || true`
- **During long-running tasks** (>2 minutes estimated): renew lease periodically:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh renew {task_id} 2>/dev/null || true`
- Check for expired leases before acquiring: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/lease-lock.sh check {task_id} {claimed_files...} 2>/dev/null || true`
...
```bash
RESULT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/two-phase-complete.sh {task_id} {phase} {plan} {contract_path} {evidence...})
```
...
- Artifact registration: after each file write during task execution, register the artifact:
  ```bash
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/artifact-registry.sh register {file_path} {event_id} {phase} {plan}
  ```
...
4. **Schema Validation — SUMMARY.md (REQ-17, graduated, always-on):**
   - Validate SUMMARY.md frontmatter: `VALID=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/validate-schema.sh summary {summary_path} 2>/dev/null || echo "valid")`
...
**Control Plane QA context:** If ``!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh` exists:
  `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/control-plane.sh compile {phase} 0 0 --role=qa --phase-dir={phase-dir} 2>/dev/null || true`
...
`bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/compile-context.sh {phase} qa {phases_dir}`
...
2. Log event: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh shutdown_sent {phase} team={team_name} targets={count} 2>/dev/null || true`
...
4. Log event: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh shutdown_received {phase} team={team_name} approved={count} rejected={count} 2>/dev/null || true`
...
2. **Merge worktree branch:**
   `MERGE_RESULT=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-merge.sh {phase} {plan} 2>/dev/null || echo "conflict")`
3. **If `MERGE_RESULT=clean`:**
   - `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-cleanup.sh {phase} {plan} 2>/dev/null || true`
   - `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/worktree-agent-map.sh clear "dev-{plan}" 2>/dev/null || true`
...
- After TeamDelete (team fully shut down), before phase_end event log:
  ```bash
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/compile-rolling-summary.sh \
...
- `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/log-event.sh phase_end {phase} plans_completed={N} total_tasks={N} 2>/dev/null || true`

**Observability Report (REQ-14):** After phase completion, if `metrics=true`:
- Generate observability report: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/metrics-report.sh {phase}`
...
[ ! -f "$PG_SCRIPT" ] && PG_SCRIPT="`!`echo $CLAUDE_PLUGIN_ROOT`/scripts/planning-git.sh"
if [ -f "$PG_SCRIPT" ]; then
...
Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/suggest-next.sh execute {qa-result}` and display output.
