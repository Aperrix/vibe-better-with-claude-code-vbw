...
- WARN if binary found but hook missing (check `${CLAUDE_CONFIG_DIR:-~/.claude}/hooks/rtk-rewrite.sh` and `~/.config/claude-code/hooks/rtk-rewrite.sh` and `~/.claude/hooks/rtk-rewrite.sh`): "Binary found, hook missing. Run: rtk init -g"
