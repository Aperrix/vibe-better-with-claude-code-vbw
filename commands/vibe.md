...
"⚠ Phase detection failed. Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/phase-detect.sh` manually to debug."
...
- **B1: PROJECT.md** -- If $ARGUMENTS provided (excluding flags), use as description. Otherwise ask name + core purpose. Then call:
  ```
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/bootstrap/bootstrap-project.sh .vbw-planning/PROJECT.md "$NAME" "$DESCRIPTION"
  ```
...
- **B2: REQUIREMENTS.md (Discovery)** -- Behavior depends on DISCOVERY_DEPTH:
  - **B2.1: Domain Research (if not skip):** If DISCOVERY_DEPTH != skip:
   ...
   2. Resolve Scout model via `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-model.sh scout .vbw-planning/config.json `!`echo $CLAUDE_PLUGIN_ROOT`/config/model-profiles.json` and Scout max turns via `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh scout .vbw-planning/config.json "$(jq -r '.effort // \"balanced\"' .vbw-planning/config.json 2>/dev/null)"`
  ...
  - **B2.2: Discussion Engine** -- Read ``!`echo $CLAUDE_PLUGIN_ROOT`/references/discussion-engine.md` and follow its protocol.
  ...
  - **After discovery (all depths):** Call:
    ```
    bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/bootstrap/bootstrap-requirements.sh .vbw-planning/REQUIREMENTS.md .vbw-planning/discovery.json .vbw-planning/domain-research.md
    ```
...
- **B3: ROADMAP.md** -- Suggest 3-5 phases from requirements. If `.vbw-planning/codebase/META.md` exists, read PATTERNS.md, ARCHITECTURE.md, and CONCERNS.md (whichever exist) from `.vbw-planning/codebase/`. Each phase: name, goal, mapped reqs, success criteria. Write phases JSON to temp file, then call:
  ```
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/bootstrap/bootstrap-roadmap.sh .vbw-planning/ROADMAP.md "$PROJECT_NAME" /tmp/vbw-phases.json
  ```
...
- **B4: STATE.md** -- Extract project name, milestone name, and phase count from earlier steps. Call:
  ```
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/bootstrap/bootstrap-state.sh .vbw-planning/STATE.md "$PROJECT_NAME" "$MILESTONE_NAME" "$PHASE_COUNT"
  ```
...
- **B6: CLAUDE.md** -- Extract project name and core value from PROJECT.md. If root CLAUDE.md exists, pass it as EXISTING_PATH for section preservation. Call:
  ```
  bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/bootstrap/bootstrap-claude.sh CLAUDE.md "$PROJECT_NAME" "$CORE_VALUE" [CLAUDE.md]
  ```
...
2. Read ``!`echo $CLAUDE_PLUGIN_ROOT`/references/discussion-engine.md` and follow its protocol for the target phase.
3. Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/suggest-next.sh vibe`.
...
4. Present grouped by status (confirmed/corrected/expanded). This mode does NOT write files. For persistence: "Run `/vbw:vibe --discuss {N}` to capture as CONTEXT.md." Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/suggest-next.sh vibe`.

### Mode: UAT Remediation

**Guard:** Initialized, target phase has `*-UAT.md` with `status: issues_found`.

**Chain state tracking:** This mode uses ``!`echo $CLAUDE_PLUGIN_ROOT`/scripts/uat-remediation-state.sh` to persist the current stage of the remediation chain on disk. This ensures correct resumption after compaction or session restart — the chain does NOT rely on prompt memory alone.
...
4. **Read or initialize remediation stage:**
   ```bash
   STAGE=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/uat-remediation-state.sh get "$PHASE_DIR")
   ```
   - If `STAGE=none`: initialize based on severity:
     ```bash
     STAGE=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/uat-remediation-state.sh init "$PHASE_DIR" "major")
...
5. **Execute the current stage** based on `STAGE`:
   - `plan`: Route into Plan mode for the same phase, with UAT issues as required planning inputs (the UAT report serves as the scoping document — no separate discussion needed). After planning completes, advance:
     ```bash
     bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/uat-remediation-state.sh advance "$PHASE_DIR"
     ```
...
4. If the user chooses remediation: create remediation phases via script — one per affected milestone phase:
   ...
   for dir in "${UAT_DIRS[@]}"; do
     bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/create-remediation-phase.sh .vbw-planning "$dir"
   done
   ```
...
5. If the user chooses start-fresh: persist acknowledgement markers for all affected archived phases before continuing:
   ...
   bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/mark-milestone-remediated.sh .vbw-planning "$TARGET_PHASE_DIRS"
   ```
...
3. **Research persistence (REQ-08, graduated):** If effort != turbo:
   ...
   - **If missing:** Spawn Scout agent to research the phase goal, requirements, and relevant codebase patterns. Scout returns structured findings with sections: `## Findings`, `## Relevant Patterns`, `## Risks`, `## Recommendations`. The **orchestrator** (not Scout) writes the returned findings to `{phase-dir}/{phase}-RESEARCH.md`. Scout has `disallowedTools: Write` (platform-enforced) and cannot write files. Resolve Scout model:
     ...
     SCOUT_MAX_TURNS=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh scout .vbw-planning/config.json "{effort}")
     ```
...
4. **Context compilation:** If `config_context_compiler=true`, run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/compile-context.sh {phase} lead {phases_dir}`. Include `.context-lead.md` in Lead agent context if produced.
5. **Turbo shortcut:** If effort=turbo, skip Lead. Read phase reqs from ROADMAP.md, create single lightweight PLAN.md inline.
6. **Other efforts:**
   - Resolve Lead model:
     ```bash
     LEAD_MODEL=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-model.sh lead .vbw-planning/config.json `!`echo $CLAUDE_PLUGIN_ROOT`/config/model-profiles.json)
       LEAD_MAX_TURNS=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh lead .vbw-planning/config.json "{effort}")
     if [ $? -ne 0 ]; then
       echo "$LEAD_MODEL" >&2
       exit 1
     fi
...
Read ``!`echo $CLAUDE_PLUGIN_ROOT`/references/execute-protocol.md` and follow its instructions.
...
**Steps:**
1. Read ``!`echo $CLAUDE_PLUGIN_ROOT`/commands/verify.md` protocol.
...
5. **Problem research (conditional):** If $ARGUMENTS contain a problem description (bug report, feature request, multi-sentence intent) rather than just a bare phase name:
   - Resolve Scout model: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-model.sh scout .vbw-planning/config.json `!`echo $CLAUDE_PLUGIN_ROOT`/config/model-profiles.json`
   - Resolve Scout max turns: `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/resolve-agent-max-turns.sh scout .vbw-planning/config.json "$(jq -r '.effort // "balanced"' .vbw-planning/config.json 2>/dev/null)"`
...
```bash
bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/archive-uat-guard.sh
```
...
1. **Derive milestone slug (deterministic — do NOT invent a slug):**
   ```bash
   MILESTONE_SLUG=$(bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/derive-milestone-slug.sh .vbw-planning)
   ```
...
5b. **Persist project-level state:** After archiving, run:
   ```bash
   bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/persist-state-after-ship.sh \
...
10. Present: Phase Banner with metrics (phases, tasks, commits, requirements, deviations), archive path, tag, branch status, memory status. Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/suggest-next.sh vibe`.
...
Run `bash `!`echo $CLAUDE_PLUGIN_ROOT`/scripts/suggest-next.sh vibe {result}` for Next Up suggestions.
