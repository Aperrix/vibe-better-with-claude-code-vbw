...
7. **Existing release branch:** Check local first (`git branch --list 'release/v*'`) and remote second (`git ls-remote --heads origin 'refs/heads/release/v*'`).
   ...
   - If local or remote checks return matches → **auto-cleanup** all stale release branches before proceeding. Collect all matching branch names (local + remote, deduplicated). Initialize counters: `{cleaned}=0`, `{failed}=0`. For each `release/v{version}` branch:
     ...
     - Delete remote branch: `git push origin --delete release/v{version}`. If deletion succeeds, increment `{cleaned}`. If the branch doesn't exist on remote, treat as success (already gone) and increment `{cleaned}`. If deletion fails for another reason (non-zero exit, e.g., permissions), increment `{failed}` and display: "⚠ Failed to delete remote branch `release/v{version}`." (Deleting the remote head branch auto-closes any associated PR on GitHub.)
   - If `{failed} > 0` and `{cleaned} == 0` → STOP: "All branch deletions failed. Check permissions and retry."
   - If `{failed} > 0` → display: "⚠ {failed} branch deletion(s) failed — check warnings above."
...
- Find last release commit: `git log --oneline --extended-regexp --grep="^chore.*: (release )?v[0-9]" -1`, extract hash (fallback: root commit). The `--extended-regexp` pattern matches `chore: release v{x}` (no scope), `chore(release): v{x}` (scoped), and `chore(release): release v{x}` (scoped with redundant prefix) while excluding `fix(release):` commits (anchored to `^chore`) and `chore(release): bump version` commits (the `(release )?v[0-9]` anchor requires `v` + digit immediately after the colon-space separator). Capture its date via `git log -1 --format=%Y-%m-%d {hash}` (fallback: empty string, which omits the `merged:>=` filter).
