...
if [ "$CMD" = "verify" ] && [ "$effective_result" = "issues_found" ] && [ -d "${PHASES_DIR:-}" ]; then
  ...
  if [ -n "$verify_target_phase" ]; then
    ...
    for dir in ${SN_VERIFY_DIRS[@]+"${SN_VERIFY_DIRS[@]}"}; do
      ...
      _uat=$(ls -1 "$dir"[0-9]*-UAT.md 2>/dev/null | grep -v 'SOURCE-UAT' | sort | tail -1 || true)
      if [ -f "$_uat" ]; then
  ...
  if [ -n "$verify_target_phase_dir" ]; then
    verify_target_uat=$(ls -1 "$verify_target_phase_dir"/[0-9]*-UAT.md 2>/dev/null | grep -v 'SOURCE-UAT' | sort | tail -1 || true)
  fi
